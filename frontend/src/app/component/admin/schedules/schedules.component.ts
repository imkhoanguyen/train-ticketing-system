    ...
    this.scheduleService.getAllSchedulesByRouteId(this.routeId).subscribe(
      (response: any) => {
        ...
        this.existingSchedules = this.schedules.map((schedule) => ({
          startDate: new Date(
            new Date(schedule.startDate).getTime() + 7 * 60 * 60 * 1000
          ),
          endDate: new Date(
            new Date(schedule.endDate).getTime() + 7 * 60 * 60 * 1000
          ),
        }));
    ...
    this.scheduleService.getScheduleById(this.id).subscribe(
      (response: any) => {
        ...
        this.scheduleForm.patchValue({
          ...
          startDate: new Date(
            new Date(scheduleArray.startDate).getTime() + 7 * 60 * 60 * 1000
          ),
          endDate: new Date(
            new Date(scheduleArray.endDate).getTime() + 7 * 60 * 60 * 1000
          ),
        });
      },
      (error) => {
